...
import {
  ...
  showInfo,
  showSuccess
} from '../../../../utilites/notifications.tsx'
import {
  addQueryStringToUrl,
  isObjectEmpty,
...
const SelectTickets = (props: SelectTicketsProps) => {
  ...
  const [orderInProcessOverlayVisible, setOrderInProcessOverlayVisible] =
    useState(false)
  ...
  const ticketMutation = useMutation({
    mutationFn: (orderData: TicketFormPayload) =>
      orderClientPublic.create(Number(eventId), orderData),

    onSuccess: data =>
      queryClient.invalidateQueries().then(() => {
        const url =
          '/checkout/' + eventId + '/' + data.data.short_id + '/details'
    ...
    onError: (error: any) => {
      ...
      notifications.show({
        message:
          ...
          t`Unable to create ticket. Please check the your details`,
        color: 'red'
      })
    }
  })
    ...
    tickets?.forEach(ticket => {
      ...
      ticket.prices?.forEach(priceQuantity => {
        quantitiesValues.push({
          ...
          price:
            ticket.type === 'DONATION' ? Number(priceQuantity.price) : undefined
        })
      })
  ...
  const handleTicketSelection = (
    values: Omit<TicketFormPayload, 'session_identifier'>
  ) => {
  ...
  return (
    <div
      className={'hi-ticket-widget-container'}
      ref={resizeRef}
      style={
        {
    ...
    >
              ...
              <Trans>
                ...
                <a
                  href={
                    ...
                    eventId +
                    '/' +
                    ticketMutation.data?.data.short_id +
                    '/details'
                  }
                  target={'_blank'}
      ...
      {event && ticketAreAvailable && (
        <form
          target={'__blank'}
          onSubmit={form.onSubmit(handleTicketSelection as any)}
        >
            ...
            {tickets &&
              tickets.map((ticket, ticketIndex) => {
                const quantityRange = range(
                  ...
                  ticket.max_per_order || 25
                ).map(n => n.toString())
                quantityRange.unshift('0')

                const isTicketCollapsed =
                  collapsedTickets[Number(ticket.id)] ?? ticket.start_collapsed
                ...
                return (
                  <div key={ticket.id} className={'hi-ticket-row'}>
                    <div className={'hi-title-row'}>
                      ...
                      >
                        ...
                        <span className={`hi-ticket-collapse-arrow`}>
                          <IconChevronRight
                            className={!isTicketCollapsed ? '' : 'open'}
                          />
                        </span>
                      </UnstyledButton>
                    </div>
                    ...
                    <Collapse
                      in={!isTicketCollapsed}
                      className={'hi-ticket-content'}
                    >
                      ...
                      {ticket.max_per_order &&
                        form.values.tickets &&
                        isObjectEmpty(form.errors) &&
                        form.values.tickets[ticketIndex]?.quantities.reduce(
                          (acc, { quantity }) => acc + Number(quantity),
                          0
                        ) > ticket.max_per_order && (
                          <div className={'hi-ticket-quantity-error'}>
                            <Trans>
                              The maximum numbers number of tickets for{' '}
                              {ticket.title}
                              is {ticket.max_per_order}
                            </Trans>
                          </div>
                        )}
                      ...
                      {ticket.description && (
                        <div className={'hi-ticket-description-row'}>
                          <Spoiler
                            maxHeight={87}
          ...
          <div className={'hi-footer-row'}>
            ...
            >
              {props.continueButtonText ||
                event?.settings?.continue_button_text ||
                t`Continue`}
            </Button>
          </div>
        </form>
      ...
      <div className={'hi-promo-code-row'}>
        {!showPromoCodeInput && !form.values.promo_code && (
          <Anchor
            className={'hi-have-a-promo-code-link'}
        ...
        {form.values.promo_code && (
          <div className={'hi-promo-code-applied'}>
            <span>
              <b>{form.values.promo_code}</b> {t`applied`}
            </span>
            <ActionIcon
      ...
      {showPromoCodeInput && !form.values.promo_code && (
        <Group
          className={'hi-promo-code-input-wrapper'}
          wrap={'nowrap'}
          gap={'20px'}
        >
          ...
          <Button
            ...
            className={'hi-apply-promo-code-button'}
            variant={'outline'}
