...
const EventLayout = () => {
  const data = [
    ...
    { comingSoon: 'coming-soon', label: t`Coming Soon`, icon: IconQrcode },
  ...
  const { data: eventSettings, isFetched: isEventSettingsFetched } =
    useGetEventSettings(eventId)
  ...
  const links = data.map(item => {
    ...
    if (
      ...
      eventSettings?.hide_getting_started_page
    ) {
      return null
    }
    ...
    return (
      <NavLink
        ...
        className={({ isActive }) =>
          (isActive && !item?.comingSoon ? classes.linkActive : '') +
          ' ' +
          classes.link
        }
      >
        ...
        <span>
          {item.label}
          {item?.comingSoon && (
            <Badge ml={'4px'} size={'xs'}>{t`Coming Soon`}</Badge>
          )}
        </span>
      </NavLink>
    )
    ...
    >
      ...
      <div className={classes.sidebar}>
        <div className={classes.logo}>
          <NavLink to={'/manage/events'}>
            <img
              style={{ maxWidth: '170px', margin: '20px auto' }}
              src={'/logo.svg'}
              alt={''}
            />
          </NavLink>
