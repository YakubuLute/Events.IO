...
const UserRequestDetails = () => {
  const { selectedRequest } = useContext(MessageContext)

  const { data: candidateProfile, isPending: candidateLoading } =
    useGetCandidatePublicProfile(selectedRequest?.senderId || '')

  const { data: organizationProfile, isPending: orgLoading } =
    useGetOrganizationProfileHook(selectedRequest?.senderOrganizationId || '')
